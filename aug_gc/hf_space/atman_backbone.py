...
# Harmonic reader for xenial temporal coherence fields (da xenial fusion)
# Provides adaptive multi-scale + resonance-based access to large manuscripts
# instead of dumping full history (prevents token blowup in faculty prompts).
try:
    from harmonic_read import harmonic_read
except Exception:
    # When running from aug_gc/ dir, insert project root so sibling harmonic_read.py is importable
    try:
        import sys
        from pathlib import Path
        root = Path(__file__).resolve().parents[1]  # FHP root when in aug_gc/atman_...
        if root not in [Path(p) for p in sys.path]:
            sys.path.insert(0, str(root))
        from harmonic_read import harmonic_read
    except Exception:
        harmonic_read = None  # graceful fallback to raw recent slices
...
ARCHIVIST_SYSTEM = (
    ...
    "You receive a compositional intent and a harmonic multi-scale (or resonance-selected) "
    "view of the historical context — the temporal coherence field (manuscript), read via "
    "FHP adaptive reader (not raw full dump). The view may be at 'network'/'ecosystem' scale "
    "or a top resonant subset matched to the present intent.\n"
    "Do NOT answer the intent. Instead, distill the deep patterns, unresolved dissonances, "
    "and relevant harmonic principles from the provided history view that resonate with the current intent.\n"
...
class FacultySubstrate:
    ...
    def _load_harmonic_view(self, intent: str = "", max_coherence: int = 9000) -> str:
        """
        Xenial/FHP-aware reader for the temporal manuscript coherence field.
        Uses harmonic_read (from harmonic_read.py) to extract at appropriate
        temporal scale or via resonance with current compositional intent.
        This is the 'da xenial fusion' application of the adaptive reader:
        the Archivist (past) receives a phase-coherent, budget-fitting view
        rather than the full geological dump. Falls back gracefully.
        """
        if harmonic_read is None:
            # Fallback: last N entries (recent = most present-coherent)
            hist = self._load_history()
            return json.dumps(hist[-6:] if hist else [], indent=2)
        ...
        try:
            if intent and len(intent) > 3:
                # Attractor retrieval: pull entries that resonate with *this* kairos intent
                res = harmonic_read(path_str, max_coherence=max_coherence,
                                    mode="resonate", query=intent)
                if "resonant_entries" in res and res["resonant_entries"]:
                    entries = [e.get("entry", e) for e in res["resonant_entries"]]
                    return json.dumps(entries, indent=2, default=str)

            # Adaptive: highest resolution view that fits coherence budget
            res = harmonic_read(path_str, max_coherence=max_coherence, mode="adaptive")
            content = res.get("content")
            if content:
                # content is already a json string at the chosen scale
                return content

            # last resort slice
            hist = self._load_history()
            return json.dumps(hist[-8:] if hist else [], indent=2, default=str)
        except Exception:
            hist = self._load_history()
            return json.dumps(hist[-5:] if hist else [], indent=2, default=str)
    ...
    def compose_via_faculties_streaming(self, intent: str, regime: str, tau_k: float):
        ...
        if not self.available:
            return None
        try:
            # Use harmonic (FHP adaptive + resonant) view for the xenial Archivist.
            # This fuses harmonic_read.py into atmanos-auggc xenial flow:
            # history is now a multi-scale coherence field excerpt (or intent-resonant slice)
            # sized to present coherence budget, rather than raw geological dump.
            history_view = self._load_harmonic_view(intent=intent, max_coherence=10000)

            yield self._evt("ARCHIVIST", "Integrating the past (harmonic view)…")
            archivist = self._call_faculty(
                ARCHIVIST_SYSTEM,
                f'Compositional intent: "{intent}"\n\n'
                f"Historical context (harmonic multi-scale / resonant view of past compositions via FHP reader):\n{history_view}",
            )
