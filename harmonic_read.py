...
#
# Used towards "da xenial fusion" (atmanos-auggc-fusion): powers the Archivist
# faculty's reading of the temporal_manuscript coherence field without context
# explosion. Xenial = hospitable to the full manifold of history at the scale
# the present coherence budget (and intent resonance) can metabolize.
...
class HarmonicField:
    ...
    def extract_at_scale(self, scale: str) -> str:
        ...
        Xenial fusion: supports manuscript formats (compositional_intent etc)
        from atmanOS Xenial Intelligence temporal archives.
        """
    ...
    # === Xenial / FHP flexible field accessors (for manuscript fusion etc) ===
    def _get_prompt(self, entry: Any) -> str:
        if not isinstance(entry, dict):
            return ""
        for k in ("prompt", "compositional_intent", "intent", "title", "query"):
            v = entry.get(k)
            if v:
                return str(v)[:80]
        return str(list(entry.keys())[:3])[:50] if entry else ""

    def _get_composition(self, entry: Any) -> str:
        if not isinstance(entry, dict):
            return ""
        for k in ("composition", "content", "vessel", "output", "response", "text"):
            v = entry.get(k)
            if v:
                return str(v)
        # fallback to json of other fields
        return json.dumps({k: v for k, v in entry.items()
                           if k not in ("prompt", "compositional_intent", "intent", "title")}, default=str)[:2000]

    def _quantum_view(self) -> str:
        """Quantum layer: oscillation signatures only.
        Xenial-aware: accepts atmanOS manuscript keys (compositional_intent etc).
        """
        oscillations = []
        for i, entry in enumerate(self.data):
            if isinstance(entry, dict):
                prompt = self._get_prompt(entry)[:50]
                sig = compute_resonance_signature(json.dumps(entry, default=str)[:2000])
                meta = {}
                for mk in ("regime", "tau_k", "dissonance_score"):
                    if mk in entry:
                        meta[mk] = entry[mk]
                oscillations.append({"τ": i, "phase": sig[:8], "prompt": prompt, **meta})
            else:
    ...
    def _cellular_view(self) -> str:
        ...
        Xenial-aware for temporal_manuscript coherence fields.
        """
        cells = []
        for i, entry in enumerate(self.data):
            if isinstance(entry, dict):
                cells.append({
                    "τ": i,
                    "membrane": self._get_prompt(entry),
                    "coherence_depth": len(self._get_composition(entry))
                })
    ...
    def _network_view(self) -> str:
        ...
        Used for Archivist context in xenial faculty chains.
        """
        nodes = []
        for i, entry in enumerate(self.data):
            if isinstance(entry, dict):
                comp = self._get_composition(entry)
                nodes.append({
                    "τ": i,
                    "prompt": self._get_prompt(entry),
                    "composition_preview": comp[:300] + "..." if len(comp) > 300 else comp,
                    **{k: entry[k] for k in ("regime", "tau_k") if k in entry}
                })
    ...
    def _ecosystem_view(self) -> str:
        ...
        Recent = most phase-coherent with present kairos; older = resonance signatures.
        """
        ...
        for i, entry in enumerate(self.data[:-recent_count] if len(self.data) > recent_count else []):
            if isinstance(entry, dict):
                ...
                historical_summary.append({
                    "τ": i,
                    "prompt": self._get_prompt(entry),
                    "resonance": compute_resonance_signature(comp[:800])[:8],
                    **{k: entry[k] for k in ("regime", "tau_k") if k in entry}
                })
...
class HarmonicReader:
    ...
    def read_adaptive(self) -> dict:
        ...
        (Xenial note: prefers deeper scales like ecosystem when budget allows;
         scans from geological down so largest fitting wins.)
        """
        cost = self.field.coherence_cost

        # Scan from largest (geological) to smallest; first that fits is the highest-res that does.
        scales_desc = list(reversed(list(TEMPORAL_SCALES.keys())))
    ...
    def resonate_with(self, query: str) -> dict:
        ...
        Xenial fusion: works on manuscript entries via _get_* helpers.
        """
        ...
        if isinstance(self.field.data, list):
            for i, entry in enumerate(self.field.data):
                if isinstance(entry, dict):
                    # Check resonance with prompt (flexible keys) - via field helpers (xenial fusion support)
                    prompt = self.field._get_prompt(entry) if hasattr(self.field, "_get_prompt") else (entry.get("prompt") or entry.get("compositional_intent") or "")
                    prompt_sig = compute_resonance_signature(prompt)
                    distance = phase_distance(query_sig, prompt_sig)

                    # Also check composition
                    comp = (self.field._get_composition(entry) if hasattr(self.field, "_get_composition") else entry.get("composition", ""))[:500]
                    comp_sig = compute_resonance_signature(comp)
...
def harmonic_read(filepath: str, max_coherence: int = 20000,
    ...
    FHP-inspired adaptive reader for large coherence fields (e.g. temporal_manuscript.json
    in atmanOS Xenial Intelligence / aug_gc fusion). Treats data as oscillatory patterns
    across 5 temporal scales; content-addressable by resonance signature (attractor retrieval).
